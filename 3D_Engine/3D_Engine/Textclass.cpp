	...
	D3DXMATRIX baseViewMatrix)
{
	...
	result = m_Font->Initialize(device, (char*)"../3D_Engine/data/fontdata.txt", (WCHAR*)L"../3D_Engine/data/font.dds");
	if (!result)
	{
...
bool TextClass::SetRenderCount(int count, ID3D11DeviceContext* deviceContext)
{
	...
	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence1, countString, 20, 20, 0.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
